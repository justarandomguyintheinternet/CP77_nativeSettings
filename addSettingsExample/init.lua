registerForEvent("onInit", function()
    ...
    settingsTables["switch"] = nativeSettings.addSwitch("/myMod/sub", "Switch", "Description", true, true, function(state) -- path, label, desc, currentValue, defaultValue, callback
        print("Changed SWITCH to ", state)
        -- Add in any logic you need in here, such as saving the changed to file / database
    end)

    settingsTables["sliderInt"] = nativeSettings.addRangeInt("/myMod", "Slider Int", "Description", 1, 100, 1, 50, 25, function(value) -- path, label, desc, min, max, step, currentValue, defaultValue, callback
        print("Changed SLIDER INT to ", value)
        -- Add in any logic you need in here, such as saving the changed to file / database
    end)

    settingsTables["sliderFloat"] = nativeSettings.addRangeFloat("/myMod/sub", "Slider Float", "Description", 1, 100, 0.25, "%.2f", 50, 1, function(value) -- path, label, desc, min, max, step, format, currentValue, defaultValue, callback
    ...
    settingsTables["selectorString"] = nativeSettings.addSelectorString("/myMod/sub", "String List", "Description", list, 1, 4, function(value) -- path, label, desc, elements, currentValue, defaultValue, callback
